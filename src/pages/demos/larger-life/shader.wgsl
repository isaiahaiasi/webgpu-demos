...
@compute @workgroup_size(
    ...
    // TODO: Extract to its own swappable function
    for (var y: i32 = -NeighborhoodDistance; y <= NeighborhoodDistance; y+= 1) {
        for (var x: i32 = -NeighborhoodDistance; x <= NeighborhoodDistance; x+= 1) {
            if (x == 0 && y == 0) {
                continue;
            }
